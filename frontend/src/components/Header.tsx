...
export default function Header({ onSearch, isLoading = false }: HeaderProps) {
    ...
    return (
        <header className="fixed top-0 left-0 right-0 z-50 bg-cotton/80 backdrop-blur-md">
            {/* Mobile Layout: Logo centered, search on second row */}
            <div className="md:hidden px-4 py-2">
                <div className="flex items-center justify-center">
                    <Link to="/" className="group flex items-center">
                        <img src="/logo.png" alt="KickRax Logo" className="w-10 h-10 object-contain -mt-0.5 -mr-1" />
                        <span className="font-display text-2xl tracking-wider text-noir">
                ...
                {onSearch && (
                    <form onSubmit={handleSubmit} className="mt-2">
                        <div className="relative flex items-center">
                            <Search className="absolute left-3 w-4 h-4 text-noir/70 z-10" strokeWidth={2.5} />
                            <input
                                type="text"
                                value={query}
            ...
            {/* Desktop Layout: Logo left, search inline, CAD badge right */}
            <div className="hidden md:block max-w-7xl mx-auto px-6 py-3">
                <div className="flex items-center justify-between gap-4">
                    <Link to="/" className="group flex-shrink-0 flex items-center">
                        <img src="/logo.png" alt="KickRax Logo" className="w-12 h-12 object-contain -mt-0.5 -mr-1" />
