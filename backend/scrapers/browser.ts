...
export function getBrowserlessStealthRoute(): string {
...
function buildBrowserlessEndpoint(): string {
    const base =
        BROWSERLESS_URL.startsWith("wss://") || BROWSERLESS_URL.startsWith("ws://")
            ? BROWSERLESS_URL.replace(/\/$/, "")
            : `wss://${BROWSERLESS_URL.replace(/^https?:\/\//, "").replace(/\/$/, "")}`;
    ...
    if (BROWSERLESS_IGNORE_DEFAULT_ARGS) {
        const flags = BROWSERLESS_IGNORE_DEFAULT_ARGS.split(",")
            .map((s) => s.trim())
            .filter(Boolean);
 ...
 */
async function unblockUrl(url: string, source: string = "SCRAPER"): Promise<{ browser: Browser; page: Page | null }> {
    ...
    try {
        ...
        const data = await response.json() as { browserWSEndpoint?: string };
        ...
        if (targetPage) {
            ...
            await (targetPage as any).waitForLoadState?.().catch(() => {});
            await new Promise(resolve => setTimeout(resolve, 1000));
        }
        
        return { browser, page: targetPage };
    } catch (error) {
        console.error(`[${source}] ❌ Unblock API failed: ${(error as Error)?.message || error}`);
        throw error;
    }
}
...
export async function launchBrowser(): Promise<Browser> {
    if (BROWSERLESS_API_TOKEN) {
        ...
        for (let attempt = 0; attempt <= BROWSERLESS_429_MAX_RETRIES; attempt++) {
            try {
                const browser = (await puppeteer.connect({ browserWSEndpoint: endpoint })) as Browser;
                ...
                if (
                    attempt < BROWSERLESS_429_MAX_RETRIES &&
                    /429|Too Many Requests/i.test(msg)
                ) {
                    const waitMs = BROWSERLESS_429_BACKOFF_MS * (attempt + 1);
                    console.warn(
                        `[Browserless] 429 rate limit, retrying in ${waitMs / 1000}s (attempt ${attempt + 1}/${BROWSERLESS_429_MAX_RETRIES})`
                    );
...
function setupPageLogging(page: Page, source: string = "SCRAPER"): string {
    ...
    const seenBotUrls = new Set<string>();
    ...
    (page as any)._blockingState = blockingState;
    return pageId;
}
 ...
 */
export function logBlockingSummary(page: Page, source: string = "SCRAPER"): void {
    const state = (page as any)._blockingState as BlockingState | undefined;
    if (!state) return;
 ...
 */
export async function createPage(browser: Browser, source: string = "SCRAPER"): Promise<Page> {
 ...
 */
export async function waitForCloudflareChallenge(page: Page, source: string = "SCRAPER", maxWaitMs: number = 15000): Promise<boolean> {
    try {
 ...
 */
export async function navigateWithLogging(
    page: Page,
    url: string,
    ...
    try {
            ...
            if (useUnblock && BROWSERLESS_API_TOKEN) {
                ...
                try {
                    ...
                    if (unblockPage) {
                        ...
                        return { status: () => 200, url: () => unblockPage.url() } as any;
                    }
                } catch (unblockError) {
                    console.error(`[${source}] ⚠️  Unblock API failed, continuing with normal navigation: ${(unblockError as Error)?.message}`);
                }
            }
        } else if (redirected) {
        ...
        if (hadChallenge && useUnblock && BROWSERLESS_API_TOKEN) {
            try {
                ...
                console.error(`[${source}] ⚠️  Unblock API failed: ${(unblockError as Error)?.message}`);
            }
        }
        ...
        try {
            ...
            if (title.includes("Just a moment") || title.includes("Checking your browser") || 
                ...
                const state = (page as any)._blockingState as BlockingState | undefined;
                if (state) {
                    state.cloudflareChallenge = true;
                    state.blockedPages.push({ url: finalUrl, reason: "Cloudflare challenge page content" });
                }
            }
            ...
            if (bodyText.includes("Access Denied") || bodyText.includes("Bot detected") ||
                ...
                const state = (page as any)._blockingState as BlockingState | undefined;
                if (state) {
                    state.blockedPages.push({ url: finalUrl, reason: "Blocked page content" });
                }
            }
        } catch (e) {
        ...
        console.error(`[${source}] ❌ Navigation failed after ${duration}ms: ${err?.message || error}`);
        throw error;
    }
}
 ...
 */
export async function navigateWithUnblock(
    url: string,
    options: { waitUntil?: "load" | "domcontentloaded" | "networkidle0" | "networkidle2"; timeout?: number } = {},
    source: string = "SCRAPER"
): Promise<{ browser: Browser; page: Page; response: import("puppeteer").HTTPResponse | null }> {
    if (!BROWSERLESS_API_TOKEN) {
