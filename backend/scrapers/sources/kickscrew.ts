...
export async function searchKickscrewBySku(sku: string, signal?: AbortSignal): Promise<KickscrewSizePricing | null> {
    ...
    try {
        ...
        const isDetached = (e: unknown) =>
            e instanceof Error && /detached|Target closed|Frame detached|Execution context is not available in detached/i.test(e.message);

        let productName: string;
        let extractedData: { sizes: { size: string; price: string | null; available: boolean }[]; imageUrl: string; itemCount: number };
        try {
            checkAbort(signal, "KICKSCREW");
            ...
            extractedData = await extractSizesEval();
        } catch (e) {
            if (e instanceof Error && e.message === "ABORTED") throw e;
