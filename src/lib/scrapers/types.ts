import { SneakerListing, SneakerSource } from '@/types';
...
// Current USD to CAD exchange rate (in production, fetch from an API)
export const USD_TO_CAD_RATE = 1.36;

export function convertToCAD(amount: number, currency: 'USD' | 'CAD'): number {
  if (currency === 'CAD') return amount;
  return Math.round(amount * USD_TO_CAD_RATE);
}

export function generateListingId(source: string, identifier: string): string {
...
// Comprehensive mock sneaker database shared across scrapers
...
export function searchMockDB(query: string, priceVariation: number = 0): typeof MOCK_SNEAKER_DB {
  ...
  const scored = MOCK_SNEAKER_DB.map(product => {
    ...
    for (const word of queryWords) {
      ...
      if (product.keywords.some(kw => kw.includes(word))) score += 2;
      if (product.brand.toLowerCase().includes(word)) score += 3;
      if (product.name.toLowerCase().includes(word)) score += 2;
      if (product.sku.toLowerCase().includes(word)) score += 5;
    }
