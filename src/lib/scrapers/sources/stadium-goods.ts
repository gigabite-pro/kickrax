...
/**
 * Stadium Goods Scraper
 * Scrapes publicly available sneaker listings from Stadium Goods
 */
...
export async function searchStadiumGoods(query: string): Promise<ScraperResult> {
  ...
  try {
    ...
    // Parse Stadium Goods search results
...
function getMockData(query: string): SneakerListing[] {
  const source = SOURCES['stadium-goods'];
  const matches = searchMockDB(query, 20); // Stadium Goods is premium pricing
